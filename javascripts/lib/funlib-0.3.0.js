...
function formatJson(json, { lineLength = 100, maxPrecision = 1 } = {}) {
...
function connectSegments(segments) {
  return FunAction.linkList(
    segments.flat().filter((e, i, a) => e !== a[i - 1]),
    { parent: true },
  );
}
...
function handySmooth(actions) {
  ...
  function simplifySegment(segment) {
    ...
    middle = middle.filter((e) => {
      return (
        e.at - first.at >= HANDY_MIN_INTERVAL &&
        last.at - e.at >= HANDY_MIN_INTERVAL
      );
    });
    ...
    if (middleDuration < HANDY_MIN_INTERVAL) {
      const middlePoint = minBy(middle, (e) =>
        Math.abs(e.at - middleDuration / 2),
      );
      return straigten([first, middlePoint, last]);
    }
    function straigten(segment2) {
      if (segment2.length <= 2) return segment2;
  ...
  filteredActions = simplifyLinearCurve(
    filteredActions,
    HANDY_MAX_STRAIGHT_THRESHOLD,
  );
...
function limitPeakSpeed(actions, maxSpeed) {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    for (let l = 0, r = 1; r < poss.length; l++, r++) {
      const left = peaks[l],
        right = peaks[r],
        absSpeed = Math.abs(left.speedFrom);
    ...
    const changes = Array.from({ length: poss.length }, (_, i2) => {
      ...
      return Math.sign(lchange) === Math.sign(rchange)
        ? Math.abs(lchange) > Math.abs(rchange)
          ? lchange
          : rchange
        : lchange + rchange;
    });
  ...
  for (let i = 0; i < segments.length; i++) {
    const newLeftPos = peaks[i].pos,
      newRightPos = peaks[i + 1].pos;
    const segment = segments[i];
    const leftAt = segment[0].at,
      rightAt = segment.at(-1).at;
    for (let j = 0; j < segment.length; j++) {
      segment[j].pos = lerp(
        newLeftPos,
        newRightPos,
        unlerp(leftAt, rightAt, segment[j].at),
      );
    }
  }
...
function toSvgBackgroundGradient(script, linearGradientId) {
  ...
  const lines = actionsToLines(actionsToZigzag(script.actions)).flatMap((e) => {
    ...
    const ra = Array.from({ length: N }, (_, i) => {
      return [
        new FunAction({
          at: lerp2(a.at, b.at, i / N),
          pos: lerp2(a.pos, b.pos, i / N),
        }),
        new FunAction({
          at: lerp2(a.at, b.at, (i + 1) / N),
          pos: lerp2(a.pos, b.pos, (i + 1) / N),
        }),
        s,
      ];
    });
  ...
  return `
    <g transform="${ops.transform}">
      ...
      <g class="funsvg-bgs">
        ...
        <rect class="funsvg-bg-title-drop" x="${xx[2]}" width="${xx[3]}" height="${yy[1]}" fill="#ccc" opacity="${round2(bgOpacity * 1.5)}"></rect>
        ...
        <rect class="funsvg-bg-title" x="${xx[2]}" width="${xx[3]}" height="${yy[1]}" fill="url(#${bgGradientId})" opacity="${round2(headerOpacity)}"></rect>
        <rect class="funsvg-bg-graph" x="${xx[2]}" width="${xx[3]}" y="${yy[1]}" height="${yy[3] - yy[1]}" fill="url(#${bgGradientId})" opacity="${round2(bgOpacity)}"></rect>
      </g>
...
class Funscript {
  ...
  static mergeMultiAxis(scripts) {
    ...
    return Object.entries(groups).flatMap(([_title, scripts2]) => {
      if (!scripts2) return [];
      ...
      if (axes.length === allScripts.length) {
        ...
        return new Funscript({
          actions: [],
          ...L0,
          axes: allScripts.sort(orderByAxis).filter((e) => e.id !== "L0"),
        });
      }
